...
(defn reset-db []
  ...
  (reset! free-slots (hash-map))
  (reset! occupied-slots (hash-map))
  (reset! total-slots (hash-map))
  (reset! floor-number 0))
...
(deftest free-slots-collection-test
  ...
  (is (map? (swap! free-slots assoc 1 (sorted-set)))true))
...
(deftest leave-condition-1
  ...
  (let [create-parking (parking-lot 6)
    ...
    car6
    car-exit-1)
  (is (= (contains? (get-in @free-slots [1]) 4) true)))
